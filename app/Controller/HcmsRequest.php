 ...
 */
class HcmsRequest implements RequestInterface
{
    use Macroable;
    ...
    public function normalizeQueryString(string $qs): string
    {
        ...
        return http_build_query($qs_array, '', '&', PHP_QUERY_RFC3986);
    }
    ...
    {
        //2024年09月22日 默认对加密的参数进行处理
        $data = $this->call(__FUNCTION__, func_get_args());
        $is_encrypt = $data['is_encrypt'] ?? false;
        if ($is_encrypt) {
            //如果是加密请求，则对data进行解密
            $data = $this->apiService->decryptData($data['data'] ?? '');
        }
    ...
    protected function getInputData(): array
    {
        return $this->storeParsedData(function () {
            ...
            return array_merge($data, $this->getQueryParams());
        });
    }
